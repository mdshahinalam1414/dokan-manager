...
// ---------- ক্যাশিং (সাইট দ্রুত করার জন্য) ----------
// ঘন ঘন-লোড হওয়া "list/dashboard" টাইপের একশনগুলো কিছু সেকেন্ড ক্যাশে রাখা হয়,
// যাতে বারবার পুরো শিট স্ক্যান করতে না হয়। যেকোনো "add/edit/delete/update" হলে
// ক্যাশ ভার্সন বেড়ে যায়, ফলে পুরনো ক্যাশ আর কখনো ব্যবহার হয় না (তথ্য সবসময় সঠিক থাকে)।

var CACHEABLE_ACTIONS = [
  'dashboard', 'staff_dashboard', 'list_capital', 'list_income_expense', 'list_shareholders',
  'list_shareholder_payments', 'list_inventory', 'list_sales', 'list_receivable_payable',
  'list_parties', 'get_settings', 'shareholder_distribution', 'monthly_summary'
];
var WRITE_ACTIONS = [
  'add_capital', 'edit_capital', 'delete_capital', 'add_income_expense',
  'add_shareholder', 'add_shareholder_payment', 'edit_shareholder_payment', 'delete_shareholder_payment',
  'add_product', 'stock_purchase', 'stock_adjust',
  'add_sale', 'edit_sale', 'delete_sale',
  'add_receivable_payable', 'add_payment_rp', 'add_party',
  'update_settings', 'add_user', 'update_user_password'
];
var CACHE_TTL_SECONDS = 300; // ৫ মিনিট
...
function bumpCacheVersion_() {
  var v = Number(getCacheVersion_()) + 1;
  PropertiesService.getScriptProperties().setProperty('CACHE_VERSION', String(v));
}
...
function cacheSet_(key, value) {
  try { CacheService.getScriptCache().put(key, JSON.stringify(value), CACHE_TTL_SECONDS); } catch (e) { /* ignore (e.g. >100KB) */ }
}
...
function handleRequest(e) {
  ...
  try {
    ...
    // ক্যাশ থেকে সরাসরি উত্তর দেওয়া যায় কিনা চেক
    var cacheKey = null;
    if (CACHEABLE_ACTIONS.indexOf(action) !== -1) {
      cacheKey = 'v' + getCacheVersion_() + ':' + action;
    ...
    if (WRITE_ACTIONS.indexOf(action) !== -1) bumpCacheVersion_();
    if (cacheKey && out && out.ok !== false) cacheSet_(cacheKey, out);
  } catch (err) {
