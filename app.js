...
const I18N = {
  bn: {
    appName: "দোকান ম্যানেজার", login: "লগইন", loginSub: "চালিয়ে যেতে ইউজারনেম ও পাসওয়ার্ড দিন",
  ...
  en: {
    appName: "Shop Manager", login: "Login", loginSub: "Enter username & password to continue",
...
async function loadCapital() {
  ...
  document.getElementById('capitalTableBody').innerHTML = rows.slice().reverse().map(r => `
    <tr><td>${fmtDate(r.Date)}</td>
      ...
      <td>৳ ${money(r.Amount)}</td><td>${r.Method}</td><td>${r.Note || ''}</td></tr>`
  ).join('') || `<tr><td colspan="5">${t('noData')}</td></tr>`;
}
document.getElementById('capitalForm').addEventListener('submit', async (e) => {
  e.preventDefault(); const f = e.target;
  await api('add_capital', { type: f.type.value, amount: f.amount.value, method: f.method.value, note: f.note.value });
...
async function loadShareholders() {
  const [holders, payments] = await Promise.all([api('list_shareholders', {}), api('list_shareholder_payments', {})]);
  ...
  document.getElementById('shPaymentTableBody').innerHTML = payments.slice().reverse().map(r => `
    <tr><td>${fmtDate(r.Date)}</td><td>${r.ShareholderName}</td><td>${r.Month}</td><td>৳ ${money(r.Amount)}</td><td>${r.Method}</td><td>${r.Note || ''}</td></tr>`
  ).join('') || `<tr><td colspan="6">${t('noData')}</td></tr>`;
}
...
document.getElementById('shPaymentForm').addEventListener('submit', async (e) => {
  e.preventDefault(); const f = e.target;
  await api('add_shareholder_payment', { shareholderName: f.shareholderName.value, month: f.month.value, amount: f.amount.value, method: f.method.value, note: f.note.value });
...
async function loadSales() {
  ...
  document.getElementById('salesTableBody').innerHTML = rows.slice().reverse().map(r => `
    <tr><td>${r.InvoiceNo}</td><td>${fmtDate(r.Date)}</td><td>${r.CustomerName || ''}</td>
      ...
      <td><button class="btn secondary" onclick="reprintVoucher('${r.InvoiceNo}')">${t('printVoucher')}</button></td></tr>`
...
document.getElementById('saleForm').addEventListener('submit', async (e) => {
  ...
  const result = await api('add_sale', {
  ...
  await reprintVoucher(result.invoiceNo);
});

async function reprintVoucher(invoiceNo) {
  const sale = await api('get_sale', { invoiceNo });
  const settings = await api('get_settings', {});
  const box = document.getElementById('voucherPrintArea');
  box.innerHTML = `
    ...
    <div class="shop-sub" style="margin-top:10px;">${t('thankYou')}</div>`;
