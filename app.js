...
/* ============ API (৫ মিনিট ব্রাউজার ক্যাশ + ব্যাকগ্রাউন্ডে অটো-সিঙ্ক) ============ */
let _apiCache = {};
const CACHE_TTL_MS = 5 * 60 * 1000; // ৫ মিনিট
const CACHEABLE_FRONTEND = [
...
async function rawApi(action, payload) {
...
async function api(action, payload) {
  // read-heavy একশনগুলো ক্যাশ থেকে সাথে সাথে দেখানো হয়, আর পেছনে চুপচাপ ফ্রেশ ডেটা এনে
  // ক্যাশ আপডেট করে রাখা হয় (পরের বার আরও নতুন ডেটা দেখাবে) — এটাই "অটো-সিঙ্ক"
  if (CACHEABLE_FRONTEND.includes(action)) {
    const cached = _apiCache[action];
    const now = Date.now();
    if (cached && (now - cached.time) < CACHE_TTL_MS) {
      rawApi(action, payload).then(fresh => { _apiCache[action] = { data: fresh, time: Date.now() }; }).catch(() => {});
      return cached.data;
    }
    const fresh = await rawApi(action, payload);
    _apiCache[action] = { data: fresh, time: Date.now() };
    return fresh;
  }
  const result = await rawApi(action, payload);
  // নিজে কোনো এন্ট্রি যোগ/এডিট/ডিলিট করলে সাথে সাথেই ক্যাশ মুছে যাবে, যাতে নিজের করা
  // পরিবর্তনটা সবসময় সাথে সাথে দেখা যায় (৫ মিনিট অপেক্ষা করতে হবে না)
  if (WRITE_FRONTEND.includes(action)) _apiCache = {};
  return result;
}
